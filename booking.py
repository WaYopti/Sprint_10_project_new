...
def get_token():
    response = requests.post(url=config.URL + config.AUTH, json=data.body_auth)
...
body = helper.change_body_create('ffff', 'pochka')
response = create_booking(body).status_code
...
# body_test["firstname"] = 'fff'
body = helper.change_body_create('Jjjjj', 'dochka')
response = create_booking(body).status_code
# print(create_booking(body_test).status_code)
print(response)
